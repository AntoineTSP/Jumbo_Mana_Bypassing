...
class CustomEnv(gym.Wrapper):
    def __init__(self, env, custom_rewards, vertices, map_size, obstacle_vertices, be_sneaky, custom_goal
                    , custom_holes, state =None):
        ...
        self.be_sneaky= be_sneaky
        self.custom_goal = custom_goal
        self.custom_holes = custom_holes
    ...
    def custom_step(self, action, custom_rewards, vertices, map_size, obstacle_vertices, be_sneaky, custom_goal, state
        ...
        print("Action")
        print(action)
        new_state, reward, done, truncated, info = self.original_step(int(action))
        ...
        if be_sneaky:
            if grid_index_to_tuple(new_state, map_size) in custom_holes:
                ...
                if is_inside_triangle(state, vertices, map_size) and new_state == tuple_to_grid_index(custom_goal,
                                                                                                      map_size):  # If the agent was directly seen and will converge to the goal
                    custom_reward = -1000
                ...
                if is_inside_triangle(new_state, vertices, map_size):
                    if new_state == tuple_to_grid_index(custom_goal, map_size):
                        if is_inside_triangle(state, vertices, map_size):
                            ...
                            custom_reward = custom_rewards.get(new_state,
                                                               0)  # If the agent reachs the goal while not being in the visible triangular
                    else:
                        for obstacle in obstacle_vertices:
                            if is_behind_obstacle(new_state, vertices, obstacle, map_size):
                                ...
                                if not is_behind_obstacle(state, vertices, obstacle,
                                                          map_size):  # Penalizes staying in a visible area
                                    custom_reward = -0.1 * 1 / (
                                                1e-8 + distance_two_point(grid_index_to_tuple(new_state, map_size),
                                                                          custom_goal))
    ...
    # def custom_step(self, action, custom_rewards, vertices, map_size, obstacle_vertices, be_sneaky, custom_goal, state, custom_holes, nb_be_hidden):
    #     """Your custom step logic here"""
    #     new_state, reward, done, truncated, info = self.original_step(action)
    #     # Customize the reward or perform additional logic based on your requirements
    #     return new_state, reward, done, truncated, info


    def step(self, action, custom_rewards, vertices, map_size, obstacle_vertices, be_sneaky,
             custom_goal, state, custom_holes, nb_be_hidden):
        return self.custom_step(action, custom_rewards, vertices, map_size, obstacle_vertices, be_sneaky, custom_goal,state, custom_holes, nb_be_hidden)
...
class DeepQLearning:
    """Perform the Deep Q-learning step using Stable Baselines3"""
    def __init__(self, learning_rate, gamma, state_size, action_size, map_size, env):
    ...
    #     """Create and initialize the DQN model"""
    #     self.model = DQN('MlpPolicy', env, learning_rate=self.learning_rate, gamma=self.gamma, verbose=1)

    def update(self, state, action, reward, new_state):
        """Update the DQN model with a single step"""
        obs = np.zeros((1, self.state_size))  # Assuming state is a 1D array
        obs[0, state] = 1  # One-hot encode the current state

        new_obs = np.zeros((1, self.state_size))  # Assuming new_state is a 1D array
        new_obs[0, new_state] = 1  # One-hot encode the new state

        # Perform a single step of learning
        self.model.learn(total_timesteps=1, reset_num_timesteps=False)

        # You may need to adapt this part based on your specific state representation and action space
        action_values = self.model.predict(obs)[0]
        q_update = action_values[action] + self.learning_rate * (reward + self.gamma * np.max(self.model.predict(new_obs)[0]) - action_values[action])

        return q_update

    def reset_model(self):
        """Reset the DQN model to its initial parameters"""
        self.create_model()
    ...
    #     env = gym.make('FrozenLake-v1', desc=np.full((self.map_size, self.map_size), 'F', dtype='str'), is_slippery=False)
    #     env = DummyVecEnv([lambda: env])  # Wrap the environment
    #     self.model = DQN.load(filename, env=env)
...
class DeepQLearning:
    """Perform the Deep Q-learning step using Stable Baselines3"""
    def __init__(self, learning_rate, gamma, state_size, action_size,env):
        self.state_size = state_size
        self.action_size = action_size
        self.learning_rate = learning_rate
        self.gamma = gamma
    ...
    def create_model(self):
        ...
        # env = DummyVecEnv([lambda: env])  # Wrap the environment
        self.model = DQN('MlpPolicy', self.env, learning_rate=self.learning_rate, gamma=self.gamma, verbose=1)
    ...
    def update(self, state, action, reward, new_state):
        ...
        self.model.learn(total_timesteps=1, reset_num_timesteps=False)  # Perform a single step of learning

        # You may need to adapt this part based on your specific state representation and action space
        action_values = self.model.predict(obs)[0]
        q_update = action_values[int(action)] + self.learning_rate * (reward + self.gamma * np.max(self.model.predict(new_obs)[0]) - action_values[int(action)])
...
class EpsilonGreedy:
    """Choose an action while being epsilon greedy for Deep Q-Learning"""
    def __init__(self, epsilon, seed=None):
        self.epsilon = epsilon
        self.rng = np.random.default_rng(seed)

    def choose_action(self, model, obs, action_space):
        """Choose an action `a` in the current observation (s)."""
        # First, we randomize a number
        explor_exploit_tradeoff = self.rng.uniform(0, 1)
...
def run_env(env, params, learner, explorer, custom_rewards, vertices, map_size, obstacle_vertices, be_sneaky, custom_goal, custom_holes):
    """Main function that plays the game with Stable Baselines3"""
    ...
    for run in range(params.n_runs):
        learner.reset_model()  # Reset the DQN model between runs
        ...
        for episode in tqdm(episodes, desc=f"Run {run}/{params.n_runs} - Episodes", leave=False):
            ...
            while not done:
                action, _ = learner.model.predict(state, deterministic=False)
                # action = explorer.choose_action(
                #     action_space=env.action_space, state=state, qmodel=learner.model
                # )

                all_states.append(state)
                all_actions.append(action)

                new_state, reward, terminated, truncated, info, nb_be_hidden = env.step(action, custom_rewards,
                                                                                        vertices, map_size,
                                                                                        obstacle_vertices,be_sneaky,custom_goal,
                                                                                        state,custom_holes,nb_be_hidden)
                # new_state, reward, done, _ = env.step(action)

                # Convert state and new_state to one-hot encoding
                obs = np.zeros((1, params.state_size))
                obs[0, state] = 1

                new_obs = np.zeros((1, params.state_size))
                new_obs[0, new_state] = 1

                q_update = learner.update(state, int(action), reward, new_state)
            ...
            qtables[run, :, :] = learner.model.predict(obs)  # Assuming qtables represent Q-values
...
#         learner.reset_model() #Reset the Network for each run
#
#         for episode in tqdm(
...
#
#             while not done:
#                 # self, model, obs, action_space
#                 action = explorer.choose_action(
#                     model=learner.model,
#                     action_space=env.action_space, obs=state,
